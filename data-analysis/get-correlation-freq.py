...
def main():
    ...
    sentiment_data = pd.read_csv('/home/kotaro/work/metaphor-analysis-m1/data/result/verb_sentiment/verb_sentiment_0518.csv',index_col=0)
    ...
    target_sent_data = sentiment_data.loc[target_words,:]
    concat_data = pd.concat([target_freq_data,target_vm_data,target_KoO_data,target_sent_data],axis=1)
    concat_data.to_csv('/home/kotaro/work/metaphor-analysis-m1/data/result/var-and-mp/concat_data-all-0522.csv')
    ...
    sent_score_list = np.array(concat_data.loc[:,'sum'])
    ...
    norm_concat_data['norm_sent_score'] = scipy.stats.zscore(sent_score_list)
    ...
    corr = np.corrcoef([mp_list,freq_list,KoO_list,entropy_list,cos_var_list,var_list,sent_score_list])
    print(corr)
